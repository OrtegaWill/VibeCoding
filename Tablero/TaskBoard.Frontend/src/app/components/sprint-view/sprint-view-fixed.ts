...
export class SprintViewComponent implements OnInit, OnDestroy {
  ...
  getSprintStatusClass(status: SprintStatus): string {
    switch (status) {
      case SprintStatus.Planned: return 'status-planned';
  ...
  async completeSprint() {
    if (!this.sprint?.id || this.sprint.status === SprintStatus.Completed) return;
    
    if (!confirm('¿Estás seguro de que quieres completar este sprint?')) return;
    ...
    try {
      ...
      const result = await lastValueFrom(this.apiService.updateSprint(updatedSprint));
      this.sprint = result;
      this.signalrService.broadcastSprintUpdate('default', result);
